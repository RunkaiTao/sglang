...
<Note>
Backend selection applies to **blockwise FP8**, **channelwise FP8**, **MXFP8** (dense linear), and **NVFP4** GEMM. When running offline or online FP8 or FP4 quantized models, you can select the GEMM backend via `--fp8-gemm-backend` and `--fp4-gemm-backend`.
</Note>
...
**Channelwise FP8 (per-token x per-channel):** `--fp8-gemm-backend triton` also routes channelwise FP8 linear layers to Triton, not just the blockwise path above. Under `auto`, channelwise stays on CUTLASS except where a tuned Triton tile exists for the GPU and weight shape (`python/sglang/kernels/ops/quantization/configs/*dtype=fp8_w8a8_channelwise.json`, currently L40S and RTX PRO 4500 Blackwell) -- the table only holds shapes where Triton measured faster. Set `SGLANG_ENABLE_FP8_GEMM_CONFIG_TUNE=0` to ignore the table, or `SGLANG_ENABLE_FP8_GEMM_TMA=0` to keep the tuned tile but load through pointer arithmetic instead of TMA descriptors.
